# -*- coding: utf-8 -*-
"""Location: ./tests/unit/cpex/fixtures/plugins/resource_filter.py
Copyright 2025
...
class ResourceFilterPlugin(Plugin):
    ...
    async def resource_pre_fetch(
        ...
        if parsed.netloc:
            ...
            if domain_lower in blocked_domains_lower or any(
                domain_lower.endswith("." + d) for d in blocked_domains_lower
            ):
                violation = PluginViolation(
        ...
        return ResourcePreFetchResult(
            continue_processing=True, modified_payload=modified_payload, metadata={"validation": "passed"}
        )
